...
void setupKnobs() {
    knob[0] = new Knob(1, encoderButtons, KNOB1_A, KNOB1_B);
    knob[0]->setRange(ledOFF, 0, 1, MAXTEMPO / 40); knob[0]->setValue(120 / 10); // bpmMilliseconds
    knob[0]->setRange(ledOFF, 1, 1, 50);                                      // brightness
    knob[0]->setRange(ledOFF, 2, 1, 25);                                      // gate duration % of note
    knob[0]->setRange(ledON, 0, 1, MAXTEMPO / 40); knob[0]->setValue(120 / 10); // bpmMilliseconds
    knob[0]->setRange(ledON, 1, 1, 50);                                      // brightness
    knob[0]->setRange(ledON, 2, 1, 25);                                      // gate duration % of note
    knob[0]->setRange(ledFLASH, 0, 1, MAXTEMPO / 40); knob[0]->setValue(120 / 10); // bpmMilliseconds
    ...
    knob[1] = new Knob(2, encoderButtons, KNOB2_A, KNOB2_B);
    ...
    knob[2] = new Knob(3, encoderButtons, KNOB3_A, KNOB3_B);
...
void handleFunctionButtons() {
    ...
    if (funcButtons.onPress(ENTER)) {
        if (seq->isStepEditing()) {   // STEP-EDIT : INSERT REST
